...
class RecursiveCharacterChunker(BaseChunker):
    ...
    def _recursive_split(self, text: str, separators: List[str]) -> List[str]:
        ...
        if separator in text:
            ...
            # Separator not found, move to next finest separator
            return self._recursive_split(text, next_separators)

        final_splits = []
        for part in parts:
            if len(part) > self.chunk_size:
                # Part is still too large, recursively split with finer separators
    ...
    def _merge_splits_with_overlap(self, splits: List[str]) -> List[str]:
        ...
        for split in splits:
            if not current_chunk:
                ...
                # Apply overlap from tail of current chunk
