"""
Unit tests for Milestone 7 (M7) Evaluation Framework and MLflow Tracker.

Tests calculate_ir_metrics, calculate_generation_metrics, calculate_system_metrics,
MLflowTracker, and RAGEvaluator.
"""
...
class TestRAGEvaluator:
    ...
    @patch.object(MLflowTracker, "log_evaluation_run")
    def test_evaluate_pipeline(self, mock_log_run):
        mock_log_run.return_value = "run-abc-456"

        mock_pipeline = MagicMock()
        chunk = Chunk(
            chunk_id="c1", doc_id="pubmed_1", chunk_index=0, text="PCD context", token_count=2
        )
